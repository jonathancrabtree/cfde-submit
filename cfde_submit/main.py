...
def run(data_path, dcc_id, catalog, schema, output_dir, delete_dir, ignore_git, dry_run,
        test_submission, verbose, server, globus, disable_validation, bag_kwargs_file, client_state_file):
    ...
    try:
        ...
        if resp:
            start_res = cfde.start_deriva_flow(data_path, dcc_id=dcc_id, catalog_id=catalog,
                                               ...
                                               globus=globus, disable_validation=disable_validation, **bag_kwargs)
        else:
